...
def css_url(m):
    ...
    mime = mimetypes.guess_type(p.name)[0] or "image/webp"
    b64 = base64.b64encode(p.read_bytes()).decode()
    print(f"  inlined css url({rel}) ({p.stat().st_size//1024} KB)")
    return f'url("data:{mime};base64,{b64}")'

out = re.sub(r'src="(assets/[^"]+)"', inline("src"), html)
out = re.sub(r'data-fallback="(assets/[^"]+)"', inline("data-fallback"), out)
out = re.sub(r'url\(["\']?(assets/[^"\')]+)["\']?\)', css_url, out)
